export default function AccionesRapidas({
  ...
  const requiereReporteProduccion = ["Salir a descanso", "Inicio de mantenimiento", accionFinalizarPasoLabel].includes(
    accion
  );
  const disableSubmit =
    !accion || (requiereAsignacion && (!asignacionesSesion.length || !asignacionPasoFinalizarId));
  ...
  const piezasDefectuosasOnChange = handleNonNegativeChange(setPiezasDefectuosas);
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 border rounded-xl p-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              ...
              disabled={asignandoPasoManual || !sesionActivaId || !pasoManualSeleccionado?.paso?.id}
      ...
      {hayAcciones && accionCard !== "asignar-paso" && accion && (
        <form onSubmit={onOperacionSubmit} className="border rounded-xl p-4 space-y-4">
          <div className="text-sm font-medium text-gray-700">
            Acción seleccionada: <span className="text-indigo-700">{accion}</span>
          </div>
          {accion === "Finalizar sesión" && (
            <p className="text-sm text-gray-700">
              Usa el botón inferior para cerrar la sesión activa seleccionada. No se requieren más datos.
            </p>
          )}
          {accion === "Volver del descanso" && (
            <p className="text-sm text-gray-700">
              Cuando regreses del descanso, presiona el botón para notificar al sistema. No se requieren datos adicionales.
            </p>
          )}
          {accion === "Fin de mantenimiento" && (
            <p className="text-sm text-gray-700">
              Finaliza el mantenimiento cuando la máquina esté lista para volver a operar. No se requieren datos adicionales.
            </p>
          )}
          {requiereAsignacion && (
            <div>
              <label className="block font-medium">Selecciona la asignación</label>
              {asignacionesSesion.length ? (
                <select
                  value={asignacionPasoFinalizarId}
                  onChange={(e) => setAsignacionPasoFinalizarId(e.target.value)}
                  className="w-full border rounded-full px-4 py-2"
                >
                  <option value="">Elige una asignación</option>
                  {asignacionesSesion.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.pasoOrden?.nombre ?? "Paso sin nombre"} · Orden{" "}
                      {item.pasoOrden?.orden?.codigo ?? item.pasoOrden?.ordenId ?? item.pasoOrden?.orden?.id ?? "-"}
                    </option>
                  ))}
                </select>
              ) : (
                <p className="text-sm text-gray-600">La sesión no tiene asignaciones activas para finalizar.</p>
              )}
            </div>
          )}
          {requiereReporteProduccion && (
            <>
              <div>
                <label className="block font-medium">Piezas buenas producidas</label>
                <input
                  type="number"
                  value={piezas}
                  min="0"
